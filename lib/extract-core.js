...
function pickBilibiliDashVideo(playData, referer) {
  ...
  return {
    ...
    audio_url: bestAudio ? bestAudio.baseUrl || bestAudio.base_url : '',
    referer,
  };
}

export async function extractBilibiliApiData({ url, shareText = '' }) {
  const bvid = extractBilibiliBvid(url, shareText);
  if (!bvid) return null;
  ...
  // fnval=0 returns a single low-resolution mp4 for anonymous users; it is easier to preview/download than DASH.
  const mp4Payload = await fetchJson(
    `https://api.bilibili.com/x/player/playurl?bvid=${encodeURIComponent(bvid)}&cid=${encodeURIComponent(cid)}&qn=16&fnval=0&fourk=0`,
    headers,
  );
  ...
  let dashVideo = null;
  if (!mp4Url) {
    const dashPayload = await fetchJson(
      `https://api.bilibili.com/x/player/playurl?bvid=${encodeURIComponent(bvid)}&cid=${encodeURIComponent(cid)}&qn=80&fnval=16&fourk=1`,
      headers,
    ).catch(() => null);
    dashVideo = pickBilibiliDashVideo(dashPayload?.data, pageUrl);
  }

  const cover = normalizeBilibiliImageUrl(view.pic);
  const videos = [];
  if (mp4Url) {
    videos.push({
      url: mp4Url,
...
export async function extractFromUrl({
  ...
  if (source.platform.id === 'bilibili') {
    const bilibiliData = await extractBilibiliApiData({
      url: source.url,
      shareText: source.share_text,
    });
    if (bilibiliData) {
      return {
        url: bilibiliData.url,
