...
function collectDouyinVideoUrls(root, pathParts = []) {
  ...
  if (Array.isArray(root.url_list) && /(play|download|video)/i.test(pathName)) {
    for (const url of root.url_list) {
      ...
      if (isLikelyVideoUrl(cleaned) || /(play|download)_addr/i.test(pathName)) {
  ...
  try {
    ...
    if (source.platform.id === 'douyin') {
      ...
      if (douyinSsrData) {
        ...
        genericData.content_text =
          douyinSsrData.content_text && douyinSsrData.content_text.length > genericData.content_text.length
            ? douyinSsrData.content_text
            : genericData.content_text;
        genericData.cover_image = douyinSsrData.cover_image || genericData.cover_image;
        genericData.images = mergeMedia([...(genericData.images || [])], douyinSsrData.images);
        genericData.videos = mergeMedia([...(genericData.videos || [])], douyinSsrData.videos);
      }
    }
